...
<!-- In historic site conservation and renewal, field research for data collection is vital. Lacking systematic methods and tools, it’s often inefficient and hard to improve. The rise of swarm intelligence computing suggests optimizing via task division and a network platform. Drawing on field experience, this study builds a network platform, explores a multi-device collaborative, real-time, efficient method, boosts research efficiency, and advances the crowdsourcing model for historic site renewal. -->

<h5 class="chapter-heading-left">方法框架</h5>

研究主要解决中国城镇历史街区人工采集数据的问题——这一环节通常是更新设计中不可避免的初步步骤，且需要严谨的团队协作。随着数字技术的发展，可构建工作群体内实时数据交互的工作流程，并开发相应的在线信息平台。然而当下，历史街区实地调研仍高度依赖分散的工作人员，包含涉及各类数据采集与评分的繁琐流程，这常导致混乱，例如评分标准不一、建筑标签冲突及调研重点误判。基于中国历史街区的特征及克罗普夫（Kropf）与马歇尔（Marshall）提出的城市形态理论，本研究建立了相关数据架构。通过在中国某历史街区的实践经验，组织构建了紧密协作的工作流程，并开发若干网页编程演示案例以验证方案可行性。经大量尝试后，确立了一套相对稳定高效的工作顺序：
...
- data prep/upload;
- pathfinding & building-path linking;
- photo/basic info recording;
- building scoring;
- household survey. -->

<h5 class="chapter-heading-left">实践应用</h5>
