...
<div class="plainpassage-brief">研究聚焦于生成式城市设计领域，特别是针对高校生活区的建筑布局问题。该研究创新性地采用二次规划（Quadratic Programming, QP）作为数学编程工具，结合通用求解器（如Gurobi）来实现设计方案的自动生成。研究核心在于通过QP模型将复杂的城市设计约束（如建筑密度、容积率、日照间距等）转化为数学表达式，从而摆脱传统手工设计的迭代过程，提升设计效率。这种方法允许设计师专注于问题形式化而非求解过程，为处理多指标、多边界形状的城市设计问题提供了新思路。</div>
